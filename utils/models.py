...
from utils.losses import CalibratedPairwiseLogistic, InBatchSampledSoftmax, TwhinLoss


class TwhinModel(nn.Module):
    def __init__(
        self, embedding_dim, item_cardinality=25834, user_cardinality=3315, relation_cardinality=3, reg_weight=0.0
    ):
        super().__init__()
...
class ModelBackbone(nn.Module):
    ...
    def forward(self, inputs):
        ...
        products_embeddings[inputs["product_id"] != 0] = products_embeddings[
            inputs["product_id"] != 0
        ] + self.token_embeddings(inputs["product_names"]["ids"], offsets)
        ...
        candidates_embeddings = self.item_encoder(
            self.item_embeddings(inputs["candidate"]) + self.token_embeddings(inputs["candidate_names"]["ids"], offsets)
        )
...
class FinetuneModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return nn.functional.binary_cross_entropy_with_logits(
            logits_pointwise, candidates_mask.to(torch.float), reduction="mean"
        ) * 0.1 + self.criterion_pairwise(logits_pairwise, candidates_mask, candidates_lengths)
