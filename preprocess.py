...
from typing import Dict, List, Any, Generator, Iterable, Tuple, Optional
import os
import argparse
...
class Reducer(ABC):
    ...
    def __init__(self, min_length: int, max_length: int, lag: int, timesplit: int, 
...
class Preprocessor:
    ...
    def preprocess_data(self) -> None:
        ...
        self._data = self._data.with_columns(
            pl.col('user_id').replace(mapping_user_ids).alias('user_id')
        )
    ...
    def preprocess_twhin_data(self, time_split: int) -> Tuple[pl.DataFrame, pl.DataFrame]:
        ...
        result = self._data.select(
            ...
            pl.col('action_type').is_in([ActionType.CLICK, ActionType.CART_UPDATE, ActionType.PURCHASE])
        ).with_columns(
            (pl.col('timestamp') > time_split).alias('is_valid')
        ).select(
            pl.col('user_id', 'product_id', 'action_type', 'is_valid')
        ).with_columns(
            pl.col('action_type').replace(self.mapping_action_types).cast(pl.Int8).alias('action_type')
        )
      
        # Split into train and test based on is_valid
        train_data = result.filter(pl.col('is_valid') == False).drop('is_valid').collect()
        test_data = result.filter(pl.col('is_valid') == True).drop('is_valid').collect()
        
        logger.info(f"Preprocessed twhin data: {len(train_data)} train rows, {len(test_data)} test rows")
        return train_data, test_data

    def preprocess_pretrain_data(self,
                                 time_split: int,
                                 min_length: int = 5,
                                 max_length: int = 256, 
                                 lag_seconds: int = 86400) -> Tuple[pl.DataFrame, pl.DataFrame]:
        """
        ...
        result = prepared_data.group_by('user_id').map_groups(
            reducer,
    ...
    def _prepare_targets(self) -> pl.LazyFrame:
        ...
        return self._data.select(
            ...
            target=pl.when(pl.col('action_type') == ActionType.VIEW).then(0).otherwise(1)
        ).drop('action_type').group_by(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-train-twhin', type=str, default='train_data_twhin.parquet', 
                        help='Path to save training data for twhin')
    parser.add_argument('--output-test-twhin', type=str, default='test_data_twhin.parquet', 
                        help='Path to save test data for twhin')
    parser.add_argument('--output-train-pretrain', type=str, default='train_data_pretrain.parquet', 
                        help='Path to save training data for pretrain')
    parser.add_argument('--output-test-pretrain', type=str, default='test_data_pretrain.parquet', 
                        help='Path to save test data for pretrain')
    parser.add_argument('--output-train-finetune', type=str, default='train_data_finetune.parquet', 
                        help='Path to save training data for finetune')
    parser.add_argument('--output-test-finetune', type=str, default='test_data_finetune.parquet', 
                        help='Path to save test data for finetune')
    parser.add_argument('--time-split', type=int, default=1703451224, 
                        help='Time split for twhin, pretrain and finetune (timestamp)')
